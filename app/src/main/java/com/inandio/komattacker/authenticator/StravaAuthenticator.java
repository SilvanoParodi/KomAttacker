package com.inandio.komattacker.authenticator;
...
public class StravaAuthenticator {
    ...
    public boolean Deauthorize() {
        try {
            URI uri = new URI("https://www.strava.com/oauth/deauthorize");
            URL url = uri.toURL();
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("POST");
            conn.setRequestProperty("Accept", "application/json");
            conn.setDoOutput(true);
            conn.setRequestProperty("Authorization", "Bearer " + Common.strava.getAccessToken());
